import { BaseModel, Token, Falsey, RefreshToken, AuthorizationCode, User, Client } from 'oauth2-server';
...
import { userModel, clientModel, accessTokenModel, authorizationCodeModel } from '../schemas';
...
export class OauthModel implements BaseModel {
    public async generateToken(payload: OAuthUser, client: OAuthClient) {
        return TokenHelper.generateToken(payload, {
            expiresIn: client.accessTokenLifetime ? client.accessTokenLifetime : ConfigEnv.get('ACCESS_TOKEN_LIFE_TIME'),
            audience: client.clientId,
            issuer: ConfigEnv.get('BASE_URL')
        })
    }

    public async generateAccessToken(client: OAuthClient, user: OAuthUser, scope: string | string[]): Promise<string> {
        let userPayload = await userModel.findOne({ _id: user.id }).lean() as OAuthUser
    ...
    public async getClient(clientId: string, clientSecret: string): Promise<OAuthClient> {
        return clientModel.findOne({ clientId: clientId, clientSecret: clientSecret }).lean() as any as OAuthClient
    }

    public async saveToken(token: OAuthAccessToken, client: OAuthClient, user: OAuthUser): Promise<Token | Falsey> {
    ...
    public async getAccessToken(accessToken: string): Promise<Token | Falsey> {
        const _accessToken = await accessTokenModel.findOne({ accessToken : accessToken }).populate('user').populate('client').lean() as OAuthAccessToken;
        if (!_accessToken) {
            return false;
        }
    ...
    public async getRefreshToken(refreshToken: string): Promise<RefreshToken> {
        return accessTokenModel.findOne({ refreshToken : refreshToken }).populate('user').populate('client').lean() as OAuthAccessToken;
    }

    public async getAuthorizationCode(code: string): Promise<AuthorizationCode> {
        return authorizationCodeModel.findOne({ authorizationCode : code }).populate('user').populate('client').lean() as OAuthAuthorizationCode
    }
    ...
    public async saveAuthorizationCode(code: any, client: OAuthClient, user: User): Promise<AuthorizationCode> {
        const isLoginEnabled = await userModel.findOne({ _id : user._id, loginEnabled : true}).lean() as any;
    ...
    public async revokeToken(accessToken: Token): Promise<boolean> {
        const result = await accessTokenModel.deleteOne({ refreshToken : accessToken.refreshToken}) as any;
        return result.deletedCount > 0;
    }

    public async revokeAuthorizationCode(code: AuthorizationCode): Promise<boolean> {
        const result = await authorizationCodeModel.deleteOne({ authorizationCode : code.authorizationCode});
        return result.deletedCount > 0;
    }
    ...
    public async logout(userId: string, clientId: string): Promise<any> {
        ...
        return Promise.all([
            accessTokenModel.deleteMany({ user: new Types.ObjectId(userId), client: new Types.ObjectId(client._id) }),
